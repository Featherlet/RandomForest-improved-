...
public class TfIdfCF {
	...
	public TfIdfCF(ArrayList<Document> formatTrainDoc, ArrayList<Document> formatTestDoc, ArrayList<wordUnit> allWords){
	...
	public void calculateWeight(){
		for(int i = 0; i < allWords.size(); i++){
			int docCounter = 0, allCounter = 0;
			int classCounter = 0;
			for(int j = 0; j < 4; j++)
				if(allWords.get(i).dfc[j] != 0){
					classCounter++;
					docCounter += allWords.get(i).dfc[j];
					allCounter += classNum[j];
				}
			allWords.get(i).classCounter = classCounter;	
			double classEntropy = 2 + infoEntropy(allWords.get(i).dfc[0], allWords.get(i).dfc[1], allWords.get(i).dfc[2], allWords.get(i).dfc[3]);
			double CF = docCounter / allCounter * 1.0;
